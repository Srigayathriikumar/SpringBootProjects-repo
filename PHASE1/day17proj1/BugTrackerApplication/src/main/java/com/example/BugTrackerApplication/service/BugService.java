...
public class BugService {
    ...
    public Page<BugResponseDTO> getAllBugs(Pageable pageable) {
        return bugRepository.findAll(pageable).map(bug -> new BugResponseDTO(
                bug.getId(),
                bug.getTitle(),
                bug.getStatus(),
                bug.getAssignee(),
    ...
    public Page<BugResponseDTO> searchBugs(String title, String status, String assignee, String project, Pageable pageable) {
        return bugRepository.searchFilteredBugs(title, status, assignee, project, pageable)
                .map(bug -> new BugResponseDTO(
                        bug.getId(),
                        bug.getTitle(),
                        bug.getStatus(),
                        bug.getAssignee(),
